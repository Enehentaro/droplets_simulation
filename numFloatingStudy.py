...
def patientAverage(df:pd.DataFrame) -> pd.DataFrame:
    ...
    num_operation:int = len(df) // 5

    df_new = None

    for i in range(num_operation):
        df_inCase = df.iloc[5*i : 5*(i+1)]
        s_mean = df_inCase.mean()

        casename:str = df.index[5*i]
...
if __name__ == "__main__":
    ...
    file_list = glob.glob(f"floating_count_results/*.csv*")
    print(file_list)
    ...
    for filename in file_list:
        ...
        df_sorted = df_read.sort_index()
        print(df_sorted)
        # df_read["RoI"] = df_read["num_drop"] / 
        # print(df_read)

        if df_total is None:
            df_total = df_sorted
        else:
            df_total = pd.concat([df_total, df_sorted])

        df_ave = patientAverage(df_sorted)
        # print(df_ave)
    ...
    outputDir = "numFloating_results"
    os.makedirs(outputDir, exist_ok=True)
    df_total.to_csv(outputDir + f"/result.csv")
    df_patientAverage.to_csv(outputDir + f"/result_patientAverage.csv")
